...
offset_1=[ones(1,Nc)*Num/Fs2*fs_init(1)*gi];
...
index=round(sub_band*([1:1:Nc]-1)+1+offset_1); 
...
%Bitrate=sum(fs_init.*(log2((p_sq*Epsilon_x./(Sigma_r+PARr*p_sq*Epsilon_x*2^(-2*b0).*fs_init.^(2*bs)))./((p_sq./((prod(abs((H_freq(round(sub_band*([1:1:Nc]-1)+1+[ones(1,Nc)*Num/Fs2*fs_init(1)*gi; ones(1,Nc)*(2*Num/Fs2*fs_init(1)*(1+gi)+Num/Fs2*fs_init(2)*gi)])))').^2))'.^(1/Nc)))*Gap))))/1e3
 ...
 Fun=@(fs_init)1e6/(sum(fs_init.*(log2((p_sq*Epsilon_x./(Sigma_r+PARr*p_sq*Epsilon_x*2^(-2*b0).*fs_init.^(2*bs)))./((p_sq./((prod(abs((H_freq(round(sub_band*([1:1:Nc]-1)+1+offset_1)))').^2))'.^(1/Nc)))*Gap)))))
% 
 A=[(1+ai)*(1+gi)];
 Aeq=[];
 Beq=[];
...
[X Bitrate]= FMINCON(Fun,[50],A,Band_total,Aeq,Beq,LB,UB,[],options);
X
1e3./Bitrate

% =======================The following generate the bits distribution vs frequency=====================
offset_opm_1=[ones(1,Nc)*Num/Fs2*X(1)*gi];
%offset_opm_2=[ones(1,Nc)*Num/Fs2*X(1)*gi; ones(1,Nc)*(2*Num/Fs2*X(1)*(1+gi)+Num/Fs2*X(2)*gi)];
sub_band=2*Num*X/Fs2/Nc; 
index_opm=round(sub_band*([1:1:Nc]-1)+1+offset_opm_1); 
...
figure(2)
plot([400/4096*index_opm(1,1)+0.01,400/4096*index_opm(1,:),400/4096*index_opm(1,32)+0.01],[0,Bits_woADC(1,:),0],'--b','LineWidth',3);
hold on;
plot([400/4096*index_opm(1,1)+0.01,400/4096*index_opm(1,:),400/4096*index_opm(1,32)+0.01],[0,Bits_wADC(1,:),0],'k','LineWidth',3);
title('1 Bands','fontsize',14);
