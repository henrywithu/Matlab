...
%offset(2)=[ones(1,Nc)*Num/Fs2*fs_init(1)*gi;  ones(1,Nc)*(2*Num/Fs2*fs_init(1)*(1+gi)+Num/Fs2*fs_init(2)*gi);  ones(1,Nc)*(2*Num/Fs2*(fs_init(1)+fs_init(2))*(1+gi)+Num/Fs2*fs_init(3)*gi)]; %% three bands  
index=round(sub_band*([1:1:Nc]-1)+1+offset_2); 
...
Bitrate=sum(fs_init.*(log2((p_sq*Epsilon_x./(Sigma_r+PARr*p_sq*Epsilon_x*2^(-2*b0).*fs_init.^(2*bs)))./((p_sq./((prod(abs((H_freq(round(sub_band*([1:1:Nc]-1)+1+[ones(1,Nc)*Num/Fs2*fs_init(1)*gi; ones(1,Nc)*(2*Num/Fs2*fs_init(1)*(1+gi)+Num/Fs2*fs_init(2)*gi)])))').^2))'.^(1/Nc)))*Gap))))/1e3
 ...
 Fun=@(fs_init)1e3/(sum(fs_init.*(log2((p_sq*Epsilon_x./(Sigma_r+PARr*p_sq*Epsilon_x*2^(-2*b0).*fs_init.^(2*bs)))./((p_sq./((prod(abs((H_freq(round(sub_band*([1:1:Nc]-1)+1+offset_2)))').^2))'.^(1/Nc)))*Gap)))))
% 
 A=[(1+ai)*(1+gi) (1+ai)*(1+gi)];
 Aeq=[];
 Beq=[];
 LB=[1;1]; %% FS_min> 1M
 UB=[];
...
[X Bitrate]= FMINCON(Fun,[50;50],A,Band_total,Aeq,Beq,LB,UB,[],options);
X
1./Bitrate

% =======================The following generate the bits distribution vs frequency=====================
offset_opm=[ones(1,Nc)*Num/Fs2*X(1)*gi; ones(1,Nc)*(2*Num/Fs2*X(1)*(1+gi)+Num/Fs2*X(2)*gi)];
...
figure(2)
plot([400/4096*index_opm(1,1)+0.01,400/4096*index_opm(1,:),400/4096*index_opm(1,32)+0.01,400/4096*index_opm(2,1)-0.001,400/4096*index_opm(2,:),400/4096*index_opm(2,32)+0.01],[0,Bits_woADC(1,:),0,0,Bits_woADC(2,:),0],'--b','LineWidth',3);
hold on;
plot([400/4096*index_opm(1,1)+0.01,400/4096*index_opm(1,:),400/4096*index_opm(1,32)+0.01,400/4096*index_opm(2,1)-0.001,400/4096*index_opm(2,:),400/4096*index_opm(2,32)+0.01],[0,Bits_wADC(1,:),0,0,Bits_wADC(2,:),0],'k','LineWidth',3);
title('2 Bands','fontsize',14)
